  (function () {
    ...
    var eng = AnimationEngine({
      ...
      draw: function (engine) {
        var s = engine.state;
        engine.ctx.clearRect(0, 0, engine.w, engine.h);
        ...
        if (s.comparingIndices.length === 2) {
          R.drawInfo(engine, 'is [' + s.comparingIndices[1] + '] < [' + s.comparingIndices[0] + ']?', COLORS.comparing);
        }
        if (s.swappingIndices.length === 2) {
          R.drawInfo(engine, 'shifting [' + s.swappingIndices[0] + '] right', COLORS.swapping);
        }
      },
