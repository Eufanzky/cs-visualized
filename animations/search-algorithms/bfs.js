(function () {
  var COLORS = AppColors;

  var NUM_NODES = 10;

  /* ── Maze constants ── */
  var COLS = 20;
  var ROWS = 12;
  var startCell = { r: 1, c: 1 };
  var goalCell = { r: ROWS - 2, c: COLS - 2 };

  /* ── Graph helpers ── */

  function addEdge(adjList, a, b) {
    if (!adjList[a].includes(b)) adjList[a].push(b);
    if (!adjList[b].includes(a)) adjList[b].push(a);
  }

  function initPositions(w, h) {
    var cx = w * 0.5;
    var cy = h * 0.38;
    var rx = Math.min(w * 0.32, 200);
    var ry = Math.min(h * 0.28, 140);
    var positions = [];
    for (var i = 0; i < NUM_NODES; i++) {
      var angle = (2 * Math.PI * i) / NUM_NODES - Math.PI / 2;
      positions.push({ x: cx + rx * Math.cos(angle), y: cy + ry * Math.sin(angle) });
    }
    return positions;
  }

  function buildGraph() {
    var adjList = Array.from({ length: NUM_NODES }, function () { return []; });
    var nodes = Array.from({ length: NUM_NODES }, function (_, i) { return i; });
    for (var i = nodes.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var tmp = nodes[i]; nodes[i] = nodes[j]; nodes[j] = tmp;
    }
    for (var i = 1; i < NUM_NODES; i++) {
      addEdge(adjList, nodes[i], nodes[Math.floor(Math.random() * i)]);
    }
    var extraEdges = 4 + Math.floor(Math.random() * 4);
    for (var e = 0; e < extraEdges; e++) {
      var a = Math.floor(Math.random() * NUM_NODES);
      var b = Math.floor(Math.random() * NUM_NODES);
      if (a !== b && !adjList[a].includes(b)) addEdge(adjList, a, b);
    }
    return adjList;
  }

  /* ── Engine (manual wiring) ── */

  var eng = AnimationEngine({
    autoWireControls: false,
    playLabel: '\u25B6 Start BFS',

    generateData: function (size, eng) {
      var s = eng.state;
      if (!s.viewMode) s.viewMode = 'graph';

      if (s.viewMode === 'maze') {
        MazeRenderer.buildMaze(s, COLS, ROWS, startCell, goalCell);
        s.queue = [];
        s.statusText = 'ready';
      } else {
        s.adjList = buildGraph();
        s.nodePositions = initPositions(eng.w, eng.h);
        s.nodeStates = Array(NUM_NODES).fill('unvisited');
        s.queue = [];
        s.edgeHighlight = null;
        s.statusText = 'ready';
      }
    },

    generateSteps: function (eng) {
      var s = eng.state;

      if (s.viewMode === 'maze') {
        return generateMazeSteps(s);
      } else {
        return generateGraphSteps(s);
      }
    },

    executeStep: function (step, eng) {
      var s = eng.state;
      LofiSounds.step(step);

      if (s.viewMode === 'maze') {
        return executeMazeStep(step, eng);
      } else {
        return executeGraphStep(step, eng);
      }
    },

    draw: function (eng) {
      var s = eng.state;
      var ctx = eng.ctx;
      ctx.clearRect(0, 0, eng.w, eng.h);

      if (s.viewMode === 'maze') {
        drawMaze(eng);
      } else {
        drawGraph(eng);
      }
    },

    onComplete: function (eng) {
      eng.state.edgeHighlight = null;
    }
  });

  /* ── Graph-mode: generateSteps ── */

  function generateGraphSteps(s) {
    s.nodeStates = Array(NUM_NODES).fill('unvisited');
    s.queue = [];
    s.edgeHighlight = null;
    s.statusText = 'ready';

    var steps = [];
    var visited = new Set();
    var q = [0];
    visited.add(0);
    steps.push({ type: 'enqueue', node: 0 });

    while (q.length > 0) {
      var node = q.shift();
      steps.push({ type: 'dequeue', node: node });
      steps.push({ type: 'visit', node: node });

      var neighbors = s.adjList[node].slice().sort(function (a, b) { return a - b; });
      for (var ni = 0; ni < neighbors.length; ni++) {
        var neighbor = neighbors[ni];
        steps.push({ type: 'check-edge', from: node, to: neighbor });
        if (!visited.has(neighbor)) {
          visited.add(neighbor);
          q.push(neighbor);
          steps.push({ type: 'enqueue', node: neighbor });
        }
      }
    }
    steps.push({ type: 'done' });
    return steps;
  }

  /* ── Graph-mode: executeStep ── */

  function executeGraphStep(step, eng) {
    var s = eng.state;
    return new Promise(function (resolve) {
      if (step.type === 'enqueue') {
        s.queue.push(step.node);
        s.nodeStates[step.node] = 'queued';
        s.edgeHighlight = null;
        eng.draw();
        setTimeout(resolve, eng.getDelay());
      } else if (step.type === 'dequeue') {
        s.queue.shift();
        s.edgeHighlight = null;
        eng.draw();
        setTimeout(resolve, eng.getDelay() * 0.3);
      } else if (step.type === 'visit') {
        s.nodeStates[step.node] = 'visiting';
        s.statusText = 'visiting node ' + step.node;
        eng.draw();
        setTimeout(function () {
          s.nodeStates[step.node] = 'visited';
          eng.draw();
          setTimeout(resolve, eng.getDelay() * 0.3);
        }, eng.getDelay() * 0.5);
      } else if (step.type === 'check-edge') {
        s.edgeHighlight = { from: step.from, to: step.to };
        eng.draw();
        setTimeout(resolve, eng.getDelay() * 0.5);
      } else if (step.type === 'done') {
        s.edgeHighlight = null;
        s.statusText = 'complete \u2713';
        eng.draw();
        setTimeout(function () { LofiSounds.complete(); }, 150);
        resolve();
      }
    });
  }

  /* ── Graph-mode: draw ── */

  function drawGraph(eng) {
    var s = eng.state;
    var ctx = eng.ctx;
    var nodeRadius = 22;

    // Edges
    for (var i = 0; i < NUM_NODES; i++) {
      for (var ji = 0; ji < s.adjList[i].length; ji++) {
        var j = s.adjList[i][ji];
        if (j > i) {
          var pi = s.nodePositions[i];
          var pj = s.nodePositions[j];
          ctx.beginPath();
          ctx.moveTo(pi.x, pi.y);
          ctx.lineTo(pj.x, pj.y);

          var strokeColor = COLORS.border;
          var lineWidth = 1.5;
          if (s.edgeHighlight && ((s.edgeHighlight.from === i && s.edgeHighlight.to === j) ||
              (s.edgeHighlight.from === j && s.edgeHighlight.to === i))) {
            strokeColor = COLORS.primary;
            lineWidth = 3;
          }
          ctx.strokeStyle = strokeColor;
          ctx.lineWidth = lineWidth;
          ctx.stroke();
        }
      }
    }

    // Nodes
    for (var i = 0; i < NUM_NODES; i++) {
      var p = s.nodePositions[i];
      var state = s.nodeStates[i];

      var fillColor = COLORS.surface;
      var borderColor = COLORS.border;
      var textColor = COLORS.textMuted;
      var glow = false;

      if (state === 'queued') {
        fillColor = COLORS.accent; borderColor = COLORS.accent; textColor = COLORS.bg; glow = true;
      } else if (state === 'visiting') {
        fillColor = COLORS.primary; borderColor = COLORS.primary; textColor = COLORS.bg; glow = true;
      } else if (state === 'visited') {
        fillColor = COLORS.success; borderColor = COLORS.success; textColor = COLORS.bg;
      }

      CanvasUtils.drawNode(ctx, p.x, p.y, nodeRadius, {
        fill: fillColor,
        stroke: borderColor,
        glow: glow && fillColor,
        label: i,
        labelColor: textColor
      });
    }

    // Queue display
    drawQueueDisplay(eng);

    // Status
    ctx.fillStyle = COLORS.textMuted;
    ctx.font = '11px JetBrains Mono, monospace';
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';
    ctx.fillText('nodes=' + NUM_NODES + '  |  ' + s.statusText, 20, 20);
  }

  /* ── Queue display (shared) ── */

  function drawQueueDisplay(eng) {
    var s = eng.state;
    var ctx = eng.ctx;
    var queueY = eng.h - 50;
    var circR = 14;
    var spacing = 36;
    var startX = 60;

    ctx.fillStyle = COLORS.textMuted;
    ctx.font = '10px JetBrains Mono, monospace';
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText('queue:', 12, queueY);

    for (var i = 0; i < s.queue.length; i++) {
      var cx = startX + i * spacing;
      if (cx + circR > eng.w - 20) break;

      CanvasUtils.drawNode(ctx, cx, queueY, circR, {
        fill: COLORS.accent,
        label: s.queue[i],
        labelColor: COLORS.bg,
        labelFont: 'bold 11px JetBrains Mono, monospace'
      });
    }
  }

  /* ── Maze-mode: generateSteps ── */

  function generateMazeSteps(s) {
    // Reset cell states for a fresh run
    for (var r = 0; r < ROWS; r++) {
      for (var c = 0; c < COLS; c++) {
        if (s.mazeCellStates[r][c] !== 'wall') {
          s.mazeCellStates[r][c] = 'empty';
        }
      }
    }
    s.mazeCellStates[startCell.r][startCell.c] = 'start';
    s.mazeCellStates[goalCell.r][goalCell.c] = 'goal';
    s.queue = [];
    s.statusText = 'ready';

    var steps = [];
    var key = function (r, c) { return MazeRenderer.cellKey(r, c, COLS); };
    var visited = new Set();
    var cameFrom = {};

    var q = [{ r: startCell.r, c: startCell.c }];
    visited.add(key(startCell.r, startCell.c));
    steps.push({ type: 'enqueue', r: startCell.r, c: startCell.c });

    while (q.length > 0) {
      var cell = q.shift();
      steps.push({ type: 'dequeue', r: cell.r, c: cell.c });
      steps.push({ type: 'visit', r: cell.r, c: cell.c });

      // Goal reached — reconstruct path
      if (cell.r === goalCell.r && cell.c === goalCell.c) {
        var path = [];
        var pr = cell.r, pc = cell.c;
        while (pr !== undefined && pc !== undefined) {
          path.push({ r: pr, c: pc });
          var k = key(pr, pc);
          var prev = cameFrom[k];
          if (!prev) break;
          pr = prev.r;
          pc = prev.c;
        }
        path.reverse();
        steps.push({ type: 'path', cells: path });
        steps.push({ type: 'done' });
        return steps;
      }

      var neighbors = MazeRenderer.getNeighbors(s, cell.r, cell.c, COLS, ROWS);
      for (var ni = 0; ni < neighbors.length; ni++) {
        var nb = neighbors[ni];
        var nk = key(nb.r, nb.c);
        steps.push({ type: 'check-neighbor', fromR: cell.r, fromC: cell.c, toR: nb.r, toC: nb.c });
        if (!visited.has(nk)) {
          visited.add(nk);
          cameFrom[nk] = { r: cell.r, c: cell.c };
          q.push(nb);
          steps.push({ type: 'enqueue', r: nb.r, c: nb.c });
        }
      }
    }

    steps.push({ type: 'no-path' });
    steps.push({ type: 'done' });
    return steps;
  }

  /* ── Maze-mode: executeStep ── */

  function executeMazeStep(step, eng) {
    var s = eng.state;
    return new Promise(function (resolve) {
      var isStart = function (r, c) { return r === startCell.r && c === startCell.c; };
      var isGoal = function (r, c) { return r === goalCell.r && c === goalCell.c; };

      if (step.type === 'enqueue') {
        var label = '(' + step.r + ',' + step.c + ')';
        s.queue.push(label);
        if (!isStart(step.r, step.c) && !isGoal(step.r, step.c)) {
          s.mazeCellStates[step.r][step.c] = 'queued';
        }
        eng.draw();
        setTimeout(resolve, eng.getDelay() * 0.5);
      } else if (step.type === 'dequeue') {
        s.queue.shift();
        eng.draw();
        setTimeout(resolve, eng.getDelay() * 0.2);
      } else if (step.type === 'visit') {
        if (!isStart(step.r, step.c) && !isGoal(step.r, step.c)) {
          s.mazeCellStates[step.r][step.c] = 'visited';
        }
        s.statusText = 'visiting (' + step.r + ',' + step.c + ')';
        eng.draw();
        setTimeout(resolve, eng.getDelay());
      } else if (step.type === 'check-neighbor') {
        s.statusText = 'checking (' + step.toR + ',' + step.toC + ')';
        eng.draw();
        setTimeout(resolve, eng.getDelay() * 0.3);
      } else if (step.type === 'path') {
        var idx = 0;
        function showNext() {
          if (idx >= step.cells.length) {
            s.statusText = 'path found! length=' + step.cells.length + ' \u2713';
            eng.draw();
            setTimeout(function () { LofiSounds.complete(); }, 150);
            resolve();
            return;
          }
          var cell = step.cells[idx];
          s.mazeCellStates[cell.r][cell.c] = 'path';
          eng.draw();
          idx++;
          setTimeout(showNext, eng.getDelay() * 0.4);
        }
        s.statusText = 'reconstructing path\u2026';
        showNext();
      } else if (step.type === 'no-path') {
        s.statusText = 'no path found \u2717';
        eng.draw();
        resolve();
      } else if (step.type === 'done') {
        if (s.statusText === 'ready') {
          s.statusText = 'complete \u2713';
        }
        eng.draw();
        resolve();
      }
    });
  }

  /* ── Maze-mode: draw ── */

  function drawMaze(eng) {
    var s = eng.state;
    var ctx = eng.ctx;

    MazeRenderer.drawMaze(eng, s, {
      cols: COLS,
      rows: ROWS,
      startCell: startCell,
      goalCell: goalCell,
      statusText: s.statusText
    });

    // Queue display below the maze
    drawQueueDisplay(eng);
  }

  /* ── Manual control wiring ── */

  var btnPlay = document.getElementById('btnPlay');
  var btnStep = document.getElementById('btnStep');
  var btnReset = document.getElementById('btnReset');
  var speedSlider = document.getElementById('speedSlider');
  var btnSound = document.getElementById('btnSound');

  if (btnPlay) {
    btnPlay.addEventListener('click', function () {
      eng.togglePlay();
    });
  }

  if (btnStep) {
    btnStep.addEventListener('click', function () {
      eng.stepOnce();
    });
  }

  if (btnReset) {
    btnReset.addEventListener('click', function () {
      eng.generateData(eng.n);
    });
  }

  if (speedSlider) {
    speedSlider.addEventListener('input', function () {
      eng.setSpeed(Number(speedSlider.value));
    });
  }

  if (btnSound) {
    btnSound.addEventListener('click', function () {
      LofiSounds.toggle();
    });
  }

  /* ── View toggle ── */

  var viewBtn = document.getElementById('btnView');
  if (viewBtn) {
    viewBtn.addEventListener('click', function () {
      var s = eng.state;
      s.viewMode = s.viewMode === 'graph' ? 'maze' : 'graph';
      viewBtn.textContent = s.viewMode === 'graph' ? '\u2591 Maze' : '\u25CE Graph';
      eng.generateData(eng.n);
    });
  }

  /* ── Resize handler ── */

  window.addEventListener('resize', function () {
    var s = eng.state;
    if (s.viewMode !== 'maze') {
      s.nodePositions = initPositions(eng.w, eng.h);
    }
  });
})();
